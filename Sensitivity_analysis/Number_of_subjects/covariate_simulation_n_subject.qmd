...
300000 subjects are simulated (3000/run for 100 runs) meaning 75000 by model cohort.
...
#| message: false
here::i_am("Sensitivity_analysis/number_of_subjects/covariate_simulation_n_subject.qmd")
cov_data <- read_csv(here("Simulations/cov_distrib_from_papers.csv"))
```

For each included paper we want to simulate `r n_patient` patients. We
...
(@Johnson2023-zo). Age, serum creatinine, body weight, sex, height and
...
serum creatinine. (CRCL_MDRD_norm means CRCL in mL/min/1.73 m² used for
Mellon and CRCL_MDRD in mL/min.). Different correlation matrices are used based on sex. This way two different mutlivariate distributions are simulated based on the two matrices with a number of patients that respects the original article's proportions.

We can find correlations for the log-transformed forms of certain variables. For the covariates where a log-normal distribution fits better the log mean and standard deviation of this distribution will be used to sample log values with a multivariate normal distribution. At the end, the log variables will be retransformed to normal ones.
...
cor_matrix_Carlier_F <- readRDS("cor_matrix_Carlier_F.rds")
print(cor_matrix_Carlier_F)
ggcorrplot(cor_matrix_Carlier_F, lab = TRUE, title = "Sex: female")

cor_matrix_Carlier_M <- readRDS("cor_matrix_Carlier_M.rds")
...
-   CRCL : Normal distribution with a minimum of 10 mL/min, with mean
    `r signif(carlier_crcl$lnorm_par["meanlog"],3)` mL/min and standard
    deviation `r signif(carlier_crcl$lnorm_par["sdlog"],3)` mL/min
...
mean_WT <- as.numeric(carlier_wt$lnorm_par["meanlog"])
sd_WT <- as.numeric(carlier_wt$lnorm_par["sdlog"])
mean_BMI <- as.numeric(carlier_bmi$lnorm_par["meanlog"])
sd_BMI <- as.numeric(carlier_bmi$lnorm_par["sdlog"])
...
mean_HT = (mean_WT - mean_BMI) / 2
sd_HT_M = (-r_M * sd_WT + sqrt(r_M^2 * sd_WT^2 - sd_WT^2 + sd_BMI^2))/2 # quadratic equation with positive result
sd_HT_F = (-r_F * sd_WT + sqrt(r_F^2 * sd_WT^2 - sd_WT^2 + sd_BMI^2))/2 # quadratic equation with positive
...
# Simulate
sim_data_M <- correlated_simulation(n = 63720, means = means, cov_matrix = cov_matrix_M)
sim_data_M <- as_tibble(sim_data_M) %>%
  mutate(SEX = 0)

sim_data_F <- correlated_simulation(n = 11280, means = means, cov_matrix = cov_matrix_F)
...
ggcorrplot(cor_sim_F, lab = TRUE, title = "Carlier simulated (females)")
ggcorrplot(cor_matrix_Carlier_M, lab = TRUE, title = "Carlier original (males)")
ggcorrplot(cor_sim_M, lab = TRUE, title = "Carlier simulated (males)")
```

```{r plot-codistribution-cov-carlier, warning = FALSE, message = FALSE}
...
cor_matrix_Fournier_F <- readRDS("cor_matrix_Fournier_F.rds")
print(cor_matrix_Fournier_F)
ggcorrplot(cor_matrix_Fournier_F, lab = TRUE, title = "Sex: female")

cor_matrix_Fournier_M <- readRDS("cor_matrix_Fournier_M.rds")
...
# Simulate
sim_data_M <- correlated_simulation(n = 57120, means = means, cov_matrix = cov_matrix_M)
sim_data_M <- as_tibble(sim_data_M) %>%
  mutate(SEX = 0)

sim_data_F <- correlated_simulation(n = 17880, means = means, cov_matrix = cov_matrix_F)
sim_data_F <- as_tibble(sim_data_F) %>%
  mutate(SEX = 1)
...
ggcorrplot(cor_sim_F, lab = TRUE, title = "Fournier simulated (females)")
ggcorrplot(cor_matrix_Fournier_M, lab = TRUE, title = "Fournier original (males)")
ggcorrplot(cor_sim_M, lab = TRUE, title = "Fournier simulated (males)")
```

```{r plot-codistribution-cov-fournier, warning = FALSE, message = FALSE}
...
cor_matrix_Mellon_F <- readRDS("cor_matrix_Mellon_F.rds")
print(cor_matrix_Mellon_F)
ggcorrplot(cor_matrix_Mellon_F, lab = TRUE, title = "Sex: female")

cor_matrix_Mellon_M <- readRDS("cor_matrix_Mellon_M.rds")
...
# Simulate
sim_data_M <- correlated_simulation(n = 11160, means = means, cov_matrix = cov_matrix_M)
sim_data_M <- as_tibble(sim_data_M) %>%
  mutate(SEX = 0)

sim_data_F <- correlated_simulation(n = 63840, means = means, cov_matrix = cov_matrix_F)
sim_data_F <- as_tibble(sim_data_F) %>%
  mutate(SEX = 1)
...
ggcorrplot(cor_sim_F, lab = TRUE, title = "Mellon simulated (females)")
ggcorrplot(cor_matrix_Mellon_M, lab = TRUE, title = "Mellon original (males)")
ggcorrplot(cor_sim_M, lab = TRUE, title = "Mellon simulated (males)")
```

```{r plot-codistribution-cov-mellon, warning = FALSE, message = FALSE}
plot_codistribution_cov(sim_cov_mellon,covariate_x = "WT",covariate_y = "CREAT")
```
...
cor_matrix_Rambaud_F <- readRDS("cor_matrix_Rambaud_F.rds")
print(cor_matrix_Rambaud_F)
ggcorrplot(cor_matrix_Rambaud_F, lab = TRUE, title = "Sex: female")

cor_matrix_Rambaud_M <- readRDS("cor_matrix_Rambaud_M.rds")
...
# Simulate
sim_data_M <- correlated_simulation(n = 59520, means = means, cov_matrix = cov_matrix_M)
sim_data_M <- as_tibble(sim_data_M) %>%
  mutate(SEX = 0)

sim_data_F <- correlated_simulation(n = 15480, means = means, cov_matrix = cov_matrix_F)
sim_data_F <- as_tibble(sim_data_F) %>%
  mutate(SEX = 1)
...
ggcorrplot(cor_sim_F, lab = TRUE, title = "Rambaud simulated (females)")
ggcorrplot(cor_matrix_Rambaud_M, lab = TRUE, title = "Rambaud original (males)")
ggcorrplot(cor_sim_M, lab = TRUE, title = "Rambaud simulated (males)")
```
...
# Export to csv
write.csv(COV, "COV3000.csv", quote = F, row.names = F)
